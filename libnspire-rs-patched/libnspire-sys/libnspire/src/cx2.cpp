...
static bool readPacket(libusb_device_handle *handle, NNSEMessage *message, int maxlen)
{
	...
	if (r < 0 || transferred < sizeof(NNSEMessage))
		return false;

	const auto completeLength = ntohs(message->length);

	if (completeLength < sizeof(NNSEMessage) || completeLength > maxlen)
		return false;
...
template <typename T>
bool sendMessage(libusb_device_handle *handle, T &message)
{
...
template <typename T>
T *messageCast(NNSEMessage *message)
{
...
static bool assureReady(struct nspire_handle *nsp_handle)
{
	...
	for (int i = 100; i-- && !nsp_handle->cx2_handshake_complete;)
	{
...
int packet_send_cx2(struct nspire_handle *nsp_handle, char *data, int size)
{
	...
	else
	{
		...
		for (int i = 100; i-- && !ret && !acked;)
		{
			...
			if (message->dest == AddrMe && message->service == (StreamService | AckFlag) && message->seqno == msg->seqno)
				acked = true;
		}
